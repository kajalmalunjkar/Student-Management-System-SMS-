...
public class SMS_Operation {
	...
	public static <First> void main(String[] args)throws NumberFormatException, IOException, SQLException, ParseException{
	    ...
	    if(choice==1)
		{
			...
			PreparedStatement ps=conn.prepareStatement("insert into account (firstName ,lastName ,phoneNo ,Email , userName ,userPassword, Age ,DOB, Gender, Qualification,signature,accid,userId )"+
			"values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			String password=null;
			 ...
			 login=true;
				...
				switch(operationNumber)
				{
					...
					Double courcefee=Double.parseDouble(br.readLine());
				   ...
				   conn=MysqlConnection.getConnection();
		            ...
		            PreparedStatement ps1=conn.prepareStatement("insert into newadmission (S_Id , S_Name , P_Name , phone , email ,dob , gender , aadhar , address , state , country , course , coursefee , signature)"+  "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		            ps1.setLong(1, S_Id);
		            ps1.setString(2,S_Name);
		            ps1.setString(3, P_Name);
		            ps1.setDouble(4, phone);
			        					  ...
			        					  ps=conn.prepareStatement("select * from account");
											//ps.setString(1, userName);
			        					     ...
			        					     while(result.next())
			        					     {
			        						  ...
			        						  System.out.println("---------------------------------------------------------------------------------------------------------------");
			        						  System.out.println("Account Id:" + result.getInt("accid"));
			        						  System.out.println("Account Name:" + result.getString("accName"));
			        						  System.out.println("Account Type:" + result.getString("accType"));
			        						  System.out.println("Gender:" + result.getString("Gender"));
			        						  System.out.println("Date of Birth:" + result.getDate("dob"));
